...
- the input files are processed as outlined above in the module overview
...
- for diagnostics, the pipeline runs `MultiQC` at the end of the preprocessing module
- a detailed report of read quality and mapping stats can be found at `./results/multiqc/multiqc_report.html`
...
- input for module 2 are the processed `*.bam` files from RNA-Seq, RPF, and/or QTI experiments from module 1
- the figure shows the read number per input file color coded by type of input
- it also shows the comparison of read number before and after filtering and shifting
- the default behavior is to filter reads by length, as shorter reads are less reliable
...
plot_pca <- df_pca %>%
  ggplot(aes(
    x = PC1, y = PC1, size = PC3, color = sample_name,
    label = sample_name
  )) +
  geom_point() +
